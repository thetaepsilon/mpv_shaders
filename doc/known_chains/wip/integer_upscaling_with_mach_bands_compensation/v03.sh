...
exec mpvhk \
	...
	--glsl-shader=$(\
		...
		| svar \
			in LINEAR \
			input_transform "$(logspace 6.)" \
			process_expr "min(((correlate_gradient(a, centre, b)) / ${logrange} * 8.), vec3(1.))" \
			out gradient_v2 \
		| cs) \
	...
	--glsl-shader=$(\
		...
		./specials/mix_n_expr \
			"vec4(exp2(    log_from_safe_transport(data1.rgb)    +    (((data3.rgb - vec3(0.5)) * ${logrange}) * exp2(  vec3(-5)  +  (data2.rgb * 3.)  ))    ),1.)" \
