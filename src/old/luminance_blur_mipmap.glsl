...
vec4 hook() {
	...
	#define P(x, y) P1(x, y, 2, 4, HOOKED_tex)
	DO_POSITIONS
	#undef P

	#define P(x, y) P1(x, y, 4, 16, light_mipmap_lvl1_tex)
	DO_POSITIONS
	#undef P
	
	#define P(x, y) P1(x, y, 8, 64, light_mipmap_lvl2_tex)
	//if (ab) {
	//DO_POSITIONS
	//}
	#undef P
	...
	res = (reference + (diff * 1.0)) * 1.0;

	// debug A-B viewing mode
	//if (ab) res = reference;
	// debug mode: only show areas where the result is brighter than reference,
	// and by how much.
	//res = diff * 2;
